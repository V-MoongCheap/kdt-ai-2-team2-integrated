...
def build_report(output: Path) -> dict[str, int]:
    ...
    lines += ["", "## 현재 판정", "- 검증 완료된 Product-level 또는 Category-level 구매/판매 원자료: 0개.", "- Foodpolis는 사용자의 대시보드 신청 후 승인 대기 상태이며, 비공개 API를 추측하지 않는다.", "- LG U+는 제공처 회신 대기 상태이며, 추가 다운로드 시도를 요구하지 않는다.", "- 생산실적, 일반 시장규모, 검색·클릭 비율은 구매/판매 evidence로 승격하지 않는다.", ""]
